...
function scEventControl_init(iSeqRow) {
  scEventControl_data["descr_status_agendamento_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["id_status_agendamento_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["descr_status_agendamento_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["descr_status_agendamento_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["id_status_agendamento_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_status_agendamento_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_status_agendamento_' + iSeqRow).bind('blur', function() { sc_form_status_agendamento_id_status_agendamento__onblur(this, iSeqRow) })
                                                    .bind('change', function() { sc_form_status_agendamento_id_status_agendamento__onchange(this, iSeqRow) })
                                                    .bind('focus', function() { sc_form_status_agendamento_id_status_agendamento__onfocus(this, iSeqRow) });
  $('#id_sc_field_descr_status_agendamento_' + iSeqRow).bind('blur', function() { sc_form_status_agendamento_descr_status_agendamento__onblur(this, iSeqRow) })
                                                       .bind('change', function() { sc_form_status_agendamento_descr_status_agendamento__onchange(this, iSeqRow) })
                                                       .bind('focus', function() { sc_form_status_agendamento_descr_status_agendamento__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_status_agendamento_id_status_agendamento__onblur(oThis, iSeqRow) {
  do_ajax_form_status_agendamento_validate_id_status_agendamento_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_status_agendamento_id_status_agendamento__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_status_agendamento_id_status_agendamento__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_status_agendamento_descr_status_agendamento__onblur(oThis, iSeqRow) {
  do_ajax_form_status_agendamento_validate_descr_status_agendamento_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_status_agendamento_descr_status_agendamento__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_status_agendamento_descr_status_agendamento__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("descr_status_agendamento_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_descr_status_agendamento_(row, status);
	displayChange_field_id_status_agendamento_(row, status);
}

function displayChange_field(field, row, status) {
	if ("descr_status_agendamento_" == field) {
		displayChange_field_descr_status_agendamento_(row, status);
	}
	if ("id_status_agendamento_" == field) {
		displayChange_field_id_status_agendamento_(row, status);
	}
}

function displayChange_field_descr_status_agendamento_(row, status) {
}

function displayChange_field_id_status_agendamento_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_status_agendamento_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_status_agendamento')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
